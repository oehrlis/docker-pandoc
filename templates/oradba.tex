...
% --- Block 'ulem' (it redefines \underline/\underbar and triggers warnings)
% Prefer KOMA's scrlfile if available; otherwise fake-load ulem to prevent it.
\IfFileExists{scrlfile.sty}{%
  \usepackage{scrlfile}%
  \PreventPackageFromLoading{ulem}%
}{%
  % Fallback: pretend ulem is already loaded so nothing tries to load it
  \expandafter\def\csname ver@ulem.sty\endcsname{}%
}

% Save originals so we can restore them in case something sneaks ulem in
\makeatletter
\let\UL@orig@underline\underline
\let\UL@orig@underbar\underbar
\makeatother
...
\definecolor{title-color}{HTML}{$if(title-color)$$title-color$$else$000000$endif$}
\definecolor{subtitle-color}{HTML}{$if(subtitle-color)$$subtitle-color$$else$A100FF$endif$}
$if(footnotes-pretty)$
...
\makeatother
$else$
% Define 'none' counter for tables without captions (Pandoc compatibility)
\newcounter{none}
...
% Define custom environments for pandoc-latex-environment filter
% Map to awesomebox commands
\newenvironment{noteblock}{\begin{awesomeblock}[purple]{2pt}{\faNoteSticky}{purple}}{\end{awesomeblock}}
\newenvironment{tipblock}{\begin{awesomeblock}[green]{2pt}{\faLightbulb}{green}}{\end{awesomeblock}}
\newenvironment{warningblock}{\begin{awesomeblock}[orange]{2pt}{\faExclamationTriangle}{orange}}{\end{awesomeblock}}
\newenvironment{cautionblock}{\begin{awesomeblock}[red]{2pt}{\faFire}{red}}{\end{awesomeblock}}
\newenvironment{importantblock}{\begin{awesomeblock}[purple]{2pt}{\faExclamationCircle}{purple}}{\end{awesomeblock}}
...
% --- Robust tables (Pandoc + tabularx) ---------------------------------------
\usepackage{longtable,tabularx,booktabs,array,ragged2e}

% X column that ragged-right wraps nicely
\newcolumntype{Y}{>{\RaggedRight\arraybackslash}X}

% Slightly taller rows look better with booktabs
\setlength{\extrarowheight}{1pt}

% Fallbacks in case templates/macros call the *Combo variants*
\providecommand{\topruleCombo}{\toprule}
\providecommand{\midruleCombo}{\midrule}
\providecommand{\bottomruleCombo}{\bottomrule}
% ----------------------------------------------------------------------------- 
...
% Make \sout from ulem work via soul's \st (avoid loading ulem)
...
\KOMAoptions{
   headsepline = $if(disable-header)$false$else$true$endif$,
   footsepline = $if(disable-footer)$false$else$true$endif$,
   plainfootsepline = true,
}
\newpairofpagestyles{eisvogel-header-footer}{
  \clearpairofpagestyles
  \ihead*{$if(disable-header)$$else$$if(header-left)$$header-left$$else$$title$$endif$$endif$}
  \chead*{$if(disable-header)$$else$$if(header-center)$$header-center$$else$$endif$$endif$}
  \ohead*{$if(disable-header)$$else$$if(header-right)$$header-right$$else$$date$$endif$$endif$}
  \ifoot*{$if(disable-footer)$$else$$if(footer-left)$$footer-left$$else$$for(author)$$author$$sep$, $endfor$$endif$$endif$}
  \cfoot*{$if(disable-footer)$$else$$if(footer-center)$$footer-center$$else$$endif$$endif$}
  \ofoot*{$if(disable-footer)$$else$$if(footer-right)$$footer-right$$else$\thepage$endif$$endif$}
...
% The titlepage with a background image has other text spacing and text size
{
  ...
  \vfill
  \vskip -8em
  \noindent {\Huge \textbf{\textsf{\color{title-color}$title$}}}
  $if(subtitle)$
  \vskip 1em
  {\Large \textbf{\color{subtitle-color}$subtitle$}}
  $endif$
  \vskip 0.5em
  \text{$if(date)$$date$$else$\today$endif$$if(tvddocversion)$, Version $tvddocversion$$else$$endif$ }\\
  \noindent {\textsf{$for(author)$$author$$sep$, $endfor$} \vskip 0.6em \textsf{$date$}}
  \vfill
}
$else$
{
  \setstretch{1.4}
  \vfill
  \noindent {\Huge \textbf{\textbf{\color{title-color}$title$}}}
  $if(subtitle)$
  \vskip 1em
  {\Large \textbf{\color{subtitle-color}$subtitle$}}
  $endif$
  \vskip 0.5em
  \text{$if(date)$$date$$else$\today$endif$$if(tvddocversion)$, Version $tvddocversion$$else$$endif$ }\\
  \noindent {\textsf{$for(author)$$author$$sep$, $endfor$}}
  \vfill
}
$endif$
